-- 测试任务跟踪系统完整数据库初始化脚本（子任务功能之前的版本）
-- 创建时间: 2024年
-- 描述: 包含所有功能但不包含子任务支持的完整初始化脚本
...
-- 插入示例任务
INSERT INTO `test_tasks` (`task_name`, `task_description`, `start_date`, `expected_end_date`, `participant_count`, `man_days`, `actual_man_days`, `task_status`, `priority`, `progress_percentage`, `risk_level`, `project_name`, `module_name`, `test_type`, `assigned_to`, `created_by_user`, `department`) VALUES
('用户登录功能测试', '测试用户登录、注册、密码重置等功能', '2024-01-01', '2024-01-05', 2, 10.0, 6.5, 'COMPLETED', 'HIGH', 100, 'LOW', '用户系统', '认证模块', 'FUNCTIONAL', 3, 1, '运营商'),
('订单管理系统测试', '测试订单创建、修改、查询、取消等功能', '2024-01-02', '2024-01-08', 3, 15.0, NULL, 'IN_PROGRESS', 'MEDIUM', 60, 'MEDIUM', '业务系统', '订单模块', 'FUNCTIONAL', 4, 1, '创新业务'),
('支付接口集成测试', '测试各种支付接口的集成和异常处理', '2024-01-03', '2024-01-10', 2, 12.0, NULL, 'PLANNED', 'HIGH', 0, 'HIGH', '支付系统', '支付模块', 'INTEGRATION', 3, 1, 'RedteaReady');

-- 插入任务进度记录
INSERT INTO `task_progress` (`task_id`, `updated_by_user`, `progress_percentage`, `progress_notes`, `update_time`, `task_status`, `actual_man_days`, `risk_level`) VALUES
(1, 3, 100, '登录功能测试全部完成，发现并修复3个bug', '2024-01-05 17:00:00', 'COMPLETED', 6.5, 'LOW'),
(2, 4, 60, '订单创建和查询功能测试完成，正在进行修改功能测试', '2024-01-06 14:30:00', 'IN_PROGRESS', NULL, 'MEDIUM');

-- 插入系统配置
INSERT INTO `system_config` (`config_key`, `config_value`, `description`) VALUES
('system.name', '测试任务跟踪系统', '系统名称'),
('system.version', '1.5.0', '系统版本'),
...
-- 插入默认定时任务
INSERT INTO `scheduled_tasks` (`task_name`, `task_description`, `cron_expression`, `bean_name`, `method_name`, `enabled`, `status`, `last_execute_result`, `execute_count`) VALUES
('taskTrackingReminder', '任务跟踪表填写提醒', '0 0 9 * * MON-FRI', 'scheduledTaskService', 'executeTaskTrackingReminder', true, 'ENABLED', 'SUCCESS', 0),
('updateTaskStatuses', '更新任务状态', '0 0 2 * * ?', 'scheduledTaskService', 'executeUpdateTaskStatuses', true, 'ENABLED', 'SUCCESS', 0),
('checkExpectedCompletionTasks', '检查预期完成时间到达的任务', '0 0 * * * ?', 'scheduledTaskService', 'executeCheckExpectedCompletionTasks', true, 'ENABLED', 'SUCCESS', 0)
ON DUPLICATE KEY UPDATE 
    ...
    last_execute_result = VALUES(last_execute_result),
    updated_time = CURRENT_TIMESTAMP;
...
-- 显示初始化完成信息
SELECT 'Pre-subtask complete database initialization finished successfully!' as message;
SELECT 
    (SELECT COUNT(*) FROM users) as total_users,
    (SELECT COUNT(*) FROM test_tasks) as total_tasks,
